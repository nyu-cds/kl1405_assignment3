"""
    ...
    Reducing funcition call overhead

    original time: 90.0s
    optimized time: 56.5s
    relative speedup (R): 90.0/56.5 = 1.59 
    improvement ranking: 1, the most important improvement
"""
    ...
    for body1 in BODIES.keys():
        for body2 in BODIES.keys():
            if (body1 != body2) and not (body2 in seenit):
                ...
                # avoid calls on compute_deltas
                # (dx, dy, dz) = compute_deltas(x1, x2, y1, y2, z1, z2)
                (dx, dy, dz) = (x1-x2, y1-y2, z1-z2)
                # avoid function calls update_vs, compute_b, compute_mag
    ...
    for body1 in BODIES.keys():
        for body2 in BODIES.keys():
            if (body1 != body2) and not (body2 in seenit):
                ((x1, y1, z1), v1, m1) = BODIES[body1]
                ((x2, y2, z2), v2, m2) = BODIES[body2]
                # avoid calling too many times on functions
                # (dx, dy, dz) = compute_deltas(x1, x2, y1, y2, z1, z2)
                (dx, dy, dz) = (x1-x2, y1-y2, z1-z2)
